...
namespace Glyphfriend.GlyphCompletionProviders
{
	...
	class FontAwesomeGlyphCompletionProvider : ICssCompletionEntryGlyphProvider
	{
		...
		private static BitmapFrame _defaultGlyph = BitmapFrame.Create(new Uri("pack://application:,,,/Glyphfriend;component/Glyphs/FontAwesome/default.png", UriKind.RelativeOrAbsolute));
		...
		private void InitializeFonts(string directory)
		{
			...
			foreach (var fontFamily in Directory.GetFiles(directory, "fontawesome-webfont.ttf", SearchOption.AllDirectories))
			{
		...
		private void ParseAndGenerateMappings(string directory)
		{
			...
			var path = Directory.EnumerateFiles(directory, "*.css", SearchOption.AllDirectories).Where(f => Regex.IsMatch(f, @"font(-?)awesome(-.*)?(\.min)?\.css$")).FirstOrDefault();
			...
			if (!String.IsNullOrEmpty(path))
			{
				...
				// Read only the CSS Classes (only those with :before and beginning with .fa- and containing "content")
				var classes = Regex.Matches(css, @"\.[-]?([_a-zA-Z][_a-zA-Z0-9-]*):before|[^\0-\177]*\\[0-9a-f]{1,6}(\r\n[ \n\r\t\f])?|\\[^\n\r\f0-9a-f]*")
								   .Cast<Match>()
								   .Where(m => m.Value.StartsWith(".fa-") && m.Value.Contains("content:"))
								   .Distinct();
