...
namespace Glyphfriend.GlyphCompletionProviders
{
	...
	class IcomoonGlyphCompletionEntryGlyphProvider : ICssCompletionEntryGlyphProvider
	{
		...
		private void InitializeFonts(string directory)
		{
			...
			foreach (var fontFamily in Directory.GetFiles(directory, "icomoon*.ttf", SearchOption.AllDirectories))
			{
		...
		private void ParseAndGenerateMappings(string directory)
		{
			...
			var path = Directory.EnumerateFiles(directory, "*.css", SearchOption.AllDirectories).Where(f => Regex.IsMatch(f, @"(style|icomoon)(\-free)?(\.min)?\.css$")).FirstOrDefault();
			...
			if (!String.IsNullOrEmpty(path))
			{
				...
				var classes = Regex.Matches(css, @"\.[-]?([_a-zA-Z][_a-zA-Z0-9-]*):before|[^\0-\177]*\\[0-9a-f]{1,6}(\r\n[ \n\r\t\f])?|\\[^\n\r\f0-9a-f]*")
								   .Cast<Match>()
								   .Where(m => m.Value.StartsWith(".icon-") && m.Value.Contains("content:"))
								   .Distinct();
				...
				foreach (var mapping in mappings)
				{
					...
					if (!_fontMappings.ContainsKey(mapping.Key))
					{
						...
						var html = String.Format("<span style='font-family: \"IcoMoon-Free\",\"IcoMoon\"; display:block;'>{0}</span>", mapping.Value);
						// Build a glyphframe
