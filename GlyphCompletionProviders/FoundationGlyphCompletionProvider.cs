...
namespace Glyphfriend.GlyphCompletionProviders
{
    ...
    class FoundationCompletionEntryGlyphProvider : ICssCompletionEntryGlyphProvider
    {
        ...
        private static BitmapFrame _defaultIcon = BitmapFrame.Create(new Uri("pack://application:,,,/Glyphfriend;component/Glyphs/Foundation/foundation-logo.png", UriKind.RelativeOrAbsolute));

        // Define a pattern to handle matching any related Foundation CSS files
        private static Regex _cssFileExpression = new Regex(@"^foundation(-.*)?(\.min)?\.css$", RegexOptions.IgnoreCase | RegexOptions.Compiled);
        ...
        public ImageSource GetCompletionGlyph(string entryName, Uri sourceUri, CssNameType nameType)
        {
            ...
            if (_cssFileExpression.IsMatch(filename))
            {
                try
                {
                    // Attempt to grab an icon for the current entry by trimming off the appropriate prefix for the entry (e.g. "fi-{entry}")
